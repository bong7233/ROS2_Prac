...
ros2 service call /set_battery_percentage \
  amr_interfaces/srv/SetBatteryPercentage "{percentage: 0.95}"   # 충전 완료 → PATROL 재개
```

`PATROL → RETURN_TO_DOCK → CHARGING → PATROL` 전환은 `/robot_state`와 `/diagnostics`에서 확인할 수 있습니다.
